...
export class Store {
  ...
  async claim(taskKey, cwd, runId) { await this.init(); const claimed = []; const owner = JSON.stringify({ runId, pid: process.pid, startedAt: Date.now() }); for (const file of this.claimFiles(taskKey, cwd)) { try { const handle = await open(file, "wx", 0o600); await handle.writeFile(owner); await handle.sync(); await handle.close(); claimed.push(file); } catch (error) { if (error.code !== "EEXIST") { await Promise.all(claimed.map((x) => unlink(x).catch(() => {}))); throw error; } let prior; try { prior = JSON.parse(await readFile(file, "utf8")); const run = await this.load(prior.runId); if (["done", "cancelled", "failed"].includes(run.status)) await unlink(file); else throw new Error("active claim"); } catch (reclaim) { if (reclaim.message === "active claim") throw reclaim; try { process.kill(prior?.pid, 0); throw new Error("orphan claim owner is live"); } catch (probe) { if (probe.code !== "ESRCH") throw probe; await unlink(file); } } const handle = await open(file, "wx", 0o600); await handle.writeFile(owner); await handle.sync(); await handle.close(); claimed.push(file); } } }
