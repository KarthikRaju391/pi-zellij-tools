...
export function guardPath({ cwd, roots, paths, input, write }) {
  ...
  let canonicalTarget; try { canonicalTarget = realpathSync(lexical); } catch { const canonicalParent = existing(dirname(lexical)); canonicalTarget = resolve(canonicalParent, basename(lexical)); }
