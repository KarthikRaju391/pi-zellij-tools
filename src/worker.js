...
export class WorkerAdapter {
  constructor({ stateDir, extension = extensionPath, spawnProcess = spawn, requestTimeout = 15000 }) { this.stateDir = stateDir; this.extension = extension; this.spawnProcess = spawnProcess; this.requestTimeout = requestTimeout; this.workers = new Map(); }
  sessionDir(worker) { return join(this.stateDir, "workers", worker.id); }
  argv(worker) { const route = roleRoute[worker.role]; return ["--mode", "rpc", "--model", route.model, "--thinking", route.thinking, "--session-dir", this.sessionDir(worker), "--no-extensions", "-e", this.extension, "--no-skills", "--no-prompt-templates", "--no-themes", "--no-context-files", "--tools", roleTools[worker.role].join(",")]; }
  start(worker, onEvent) {
    ...
    const rejectPending = (error) => { for (const pending of record.pending.values()) { clearTimeout(pending.timer); pending.reject(error); } record.pending.clear(); }; const parser = new LfJsonl((event) => { if (event.type === "response" && event.id && record.pending.has(event.id)) { const pending = record.pending.get(event.id); clearTimeout(pending.timer); pending.resolve(event); record.pending.delete(event.id); return; } if (event.type === "tool_execution_end" && event.toolName === "orchestrator_report" && !event.isError) record.reports.push(event.result?.details); if (event.type === "agent_settled") record.idle = true; onEvent?.(event, record); });
    child.stdout.on("data", (chunk) => { try { parser.write(chunk); } catch (error) { rejectPending(error); onEvent?.({ type: "protocol_error", error: String(error) }, record); } }); child.stdout.on("end", () => { try { parser.end(); } catch (error) { rejectPending(error); onEvent?.({ type: "protocol_error", error: String(error) }, record); } }); child.on?.("exit", () => { rejectPending(new Error("RPC process exited")); onEvent?.({ type: "process_exit" }, record); }); child.on?.("error", (error) => { rejectPending(error); onEvent?.({ type: "protocol_error", error: String(error) }, record); }); child.stderr.on("data", (chunk) => onEvent?.({ type: "stderr", text: String(chunk) }, record)); this.workers.set(record.id, record); return record;
  }
  command(worker, type, fields = {}) { const id = idFor("command", { worker: worker.id, type, fields }); const response = new Promise((resolve, reject) => { const timer = setTimeout(() => { worker.pending.delete(id); reject(new Error(`RPC ${type} timed out`)); }, this.requestTimeout); worker.pending.set(id, { resolve, reject, timer }); }); worker.child.stdin.write(`${JSON.stringify({ id, type, ...fields })}\n`); return response; }
