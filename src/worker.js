...
export class WorkerAdapter {
  ...
  start(worker, onEvent) {
    ...
    child.stdout.on("data", (chunk) => { try { parser.write(chunk); } catch (error) { for (const pending of record.pending.values()) pending.reject(error); record.pending.clear(); onEvent?.({ type: "protocol_error", error: String(error) }, record); } }); child.stdout.on("end", () => { try { parser.end(); } catch (error) { for (const pending of record.pending.values()) pending.reject(error); record.pending.clear(); onEvent?.({ type: "protocol_error", error: String(error) }, record); } }); child.on?.("exit", () => { for (const pending of record.pending.values()) pending.reject(new Error("RPC process exited")); record.pending.clear(); onEvent?.({ type: "process_exit" }, record); }); child.stderr.on("data", (chunk) => onEvent?.({ type: "stderr", text: String(chunk) }, record)); this.workers.set(record.id, record); return record;
  }
