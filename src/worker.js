...
export class WorkerAdapter {
  ...
  start(worker, onEvent) {
    ...
    const parser = new LfJsonl((event) => { if (event.type === "response" && event.id && record.pending.has(event.id)) { record.pending.get(event.id)(event); record.pending.delete(event.id); return; } if (event.type === "tool_execution_end" && event.toolName === "orchestrator_report" && !event.isError) record.reports.push(event.result?.details); if (event.type === "agent_settled") record.idle = true; onEvent?.(event, record); });
    child.stdout.on("data", (chunk) => parser.write(chunk)); child.stdout.on("end", () => parser.end()); child.on?.("exit", () => onEvent?.({ type: "process_exit" }, record)); child.stderr.on("data", (chunk) => onEvent?.({ type: "stderr", text: String(chunk) }, record)); this.workers.set(record.id, record); return record;
  }
  command(worker, type, fields = {}) { const id = idFor("command", { worker: worker.id, type, fields }); const response = new Promise((resolve) => worker.pending.set(id, resolve)); worker.child.stdin.write(`${JSON.stringify({ id, type, ...fields })}\n`); return response; }
