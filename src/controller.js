...
export class Controller {
  ...
  async close() { this.workers.shutdown?.(); await this.lease?.release(); if (this.run && TERMINAL.has(this.run.status)) await this.store.releaseClaim(this.run.taskKey, this.run.cwd); }
  ...
  async _onWorkerEvent(stage, event, worker) {
    ...
    if (["process_exit", "stderr"].includes(event.type)) return this.event(this.fenced("recovery-wait", { reason: `worker-${stage.id}-${event.type}` }));
  ...
  async reconcile(target, decision) {
    ...
    return this.enqueue(async () => { let result; if (decision === "confirmed-applied") { const kind = this.run.effects[target]?.kind ?? "agent"; if (!this.effects.prove) throw new Error("exact read-only proof is unavailable"); result = await this.effects.prove(kind, this.run.effects[target]?.result); if (!result || (kind !== Effect.PUBLISH_PR && !result.head) || (kind === Effect.PUBLISH_PR && !result.pr)) throw new Error("exact proof did not establish outcome"); } await this.event(this.fenced("reconcile", { target, decision, result })); if (this.run.status === "running") await this.pump(); return this.run; });
  }
