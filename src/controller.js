...
export class Controller {
  ...
  async resume(id) {
    ...
    await this.acquire(); return this.enqueue(() => this.recover());
  }
  ...
  async close() { this.workers.shutdown?.(); await this.lease?.release(); if (this.run && ["done", "cancelled", "failed"].includes(this.run.status)) await this.store.releaseClaim(this.run.taskKey, this.run.cwd, this.run.id); }
  ...
  async _onWorkerEvent(stage, event, worker) {
    ...
    if (["process_exit", "process_error", "protocol_error"].includes(event.type) && this.run.nodes[stage.id]?.status === "running") return this.event(this.fenced("recovery-wait", { reason: `worker-${stage.id}-${event.type}` }));
  ...
  async reconcile(target, decision) {
    ...
    return this.enqueue(async () => { let result; let kind; if (decision === "confirmed-applied") { kind = this.run.effects[target]?.kind; if (kind !== Effect.PUBLISH_PR) throw new Error("confirmed-applied is allowed only for exact open PR proof"); const review = this.run.nodes["review-head"]; const approvedHead = review?.status === "approved" ? review.head : undefined; const current = approvedHead && await this.effects.head(); if (!approvedHead || current !== approvedHead) throw new Error("final review HEAD is no longer current"); if (!this.effects.prove) throw new Error("exact read-only proof is unavailable"); result = await this.effects.prove(kind, this.run.effects[target], { approvedHead }); if (!result?.pr) throw new Error("exact proof did not establish outcome"); } await this.event(this.fenced("reconcile", { target, decision, result })); if (kind === Effect.PUBLISH_PR && decision === "confirmed-applied") await this.event(this.fenced("complete", { pr: result.pr })); if (this.run.status === "running") await this.pump(); return this.run; });
  }
