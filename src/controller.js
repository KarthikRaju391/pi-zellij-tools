...
export class Controller {
  ...
  async resume(id) { this.run = await this.store.load(id); await this.store.claim(this.run.taskKey, this.run.cwd, this.run.id); this.effects = this.effectsFor(this.run.spec); if (this.run.workflow === "fix-to-pr") await this.effects.preflight?.({ allowDirty: true }); await this.acquire(); return this.enqueue(() => this.recover()); }
  ...
  async effect(stage) {
    const kind = Object.values(Effect).includes(stage.effect) ? stage.effect : Effect.CHECKS; if ([Effect.RECONCILE_PR, Effect.PUBLISH_PR].includes(kind)) { const approved = this.run.nodes["review-head"]?.head; let current; try { current = await this.effects.head(); } catch { return this.event(this.fenced("recovery-wait", { reason: "cannot-read-final-head" })); } if (!approved || current !== approved) return this.event(this.fenced("recovery-wait", { reason: "final-review-head-stale" })); } const old = this.run.effects[stage.id];
